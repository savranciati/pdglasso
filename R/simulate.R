...
#'   inside-block edges among the \eqn{q(q-1)} inside-block
#'   edges, where \eqn{q=p/2}, . This is used if the string "inside.block.edge" is a component of
...
#'   across-block edges among the potentially coloured \eqn{q(q-1)} across-block
#'   edges. This is used if the string "across.block.edge" is a component
...
pdRCON.simulate <- function(p,
  ...
  if(concent.mat){
    ...
    if(sample){
      ...
      if(sample.size==1) sample.data <- t(sample.data)
  ...
  return(list(pdColG = as.pdColG(PDCG), K = K, sample.data = sample.data))
}
...
#'
#' Randomly generates a GGM and, more specifically, the undirected graph `G` 
#' representing the model, a concentration matrix `K` adapted to `G`, 
#' that is a positive definite matrix 
#' whose zero pattern is specified by the missing edges of `G`, and a random sample
#' from a multivariate normal distribution with zero mean vector and covariance
#' matrix `Sigma`, that is the inverse of `K`.
#'
...
GenMod <- pdRCON.simulate (p,
  ...
  diag(GenMod$G) <- 0
  class(GenMod$G) <- class(GenMod$G)[-1]
  if(concent.mat){
