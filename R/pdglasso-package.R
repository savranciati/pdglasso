...
#' details on the equality constraints of pdRCON models, on submodel classes
...
#' interest, identified by the names *vertex*, *inside-block edge* and
...
#' referred to as *coloured inside-block edges*. Different types of
...
#' have equal value. These type of edges are referred to as *coloured
...
#'
#' A pair of homologous edges which are  both present
#' in the graph form what we call a *structural symmetry*. A structural symmetry can be 
#' either *uncoloured* or *coloured* and the latter is also called a *parametric symmetry*.
#' Accordingly, a pair of homologous coloured vertices is called a *vertex symmetry*. 
#' It is worth remarking that, strictly speaking, a pair of homologous missing edges 
#' may be regarded as a parametric symmetry because the associated concentrations have the
#' same, zero, value. However, we deem it convenient to use this terminology only to refer to  
#' present edges. 
#'
...
#'
#'  * If `type` contains the string `"vertex"` then vertex symmetries are
#'  allowed and, if in addition also `force.symm` contains the string
#'  `"vertex"`, then all vertices are coloured.
#'
#'  * If `type` contains the string `"inside.block.edge"` then coloured inside-block symmetries are allowed and, if in addition also `force.symm`
#'  contains the string `"inside.block.edge"`, then only coloured edges are
#'  allowed inside blocks.
#'
#'  * If `type` contains the string `"across.block.edge"` then coloured across-block symmetries are allowed and, if in addition also `force.symm`
...
#'
#' @section Model representation through a pdColG matrix:
#'
...
#'
#'  * The diagonal entries of a pdColG matrix are all equal to either 1,
#'  for uncoloured vertices, or 2, for coloured vertices.
#'
#'  * The off-diagonal entries of a pdColG matrix are equal to 0 for missing
